...
# If you use the enhanced-detector scripts to download the action localization datasets, for instance POTFD:
# ROOT_DATASET_PATH = CAFFE_PATH + "/data/POTFD/"
...
class TubeDataset(object):
    ...
    def __init__(self, dname, split=1):
        assert dname in ['POFD','THUMOS'], "Unknown dataset name"
        self.NAME = dname
        self.SPLIT = split
...
class POTFD(TubeDataset):

    def __init__(self, split=1):
        assert split == 1, "POTFD has only 1 split"
        super(UCFSports, self).__init__('POTFD', 1)

    def imfile(self, v, i):
        return os.path.join(ROOT_DATASET_PATH, "POTFD", "Frames", v, "{:0>6}.jpg".format(i))

    def flowfile(self, v, i):
        return os.path.join(ROOT_DATASET_PATH, "POTFD", "FlowBrox04", v, "{:0>6}.jpg".format(i))

    def frame_format(self, v, i):
        return os.path.join(v, "{:0>6}".format(i))

class THUMOS(TubeDataset):
    def __init__(self, split=1):
        assert 1 <= split <= 3, "THUMOS have 3 splits"
        super(JHMDB, self).__init__('THUMOS', split)

    def imfile(self, v, i):
        return os.path.join(ROOT_DATASET_PATH, "THUMOS", "Frames", v, "{:0>5}.png".format(i))

    def flowfile(self, v, i):
        return os.path.join(ROOT_DATASET_PATH, "THUMOS", "FlowBrox04", v, "{:0>5}.jpg".format(i))
...
def GetDataset(dname):
    assert dname in ['POTFD', 'THUMOS', 'UCF101'], "Unknown dataset " + dname

    if dname == 'POTFD': return POTFD()
    if dname == 'THUMOS': return THUMOS()
